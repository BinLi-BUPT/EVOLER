...
for m=1:runs
   % CPSO-H
    ...
    for i=1:s_ps
        ...
        s_initFitness(i)=power_allocation_3D(x);%****************************
    end
    s_pop=s_initPop;
    s_v=s_initV;
    ...
    for k=1:maxGen
        for j=1:D
            for i=1:ps
                ...
                temp1(j)=pop(i,j);pBest1(j)=pBest(i,j);
                x=temp1;    y=pBest1;
                ...
                fit1=power_allocation_3D(x);%*******************************
                fit2=power_allocation_3D(y);%*********************************
                %
                if fit1<fit2
                    pBestFit(i)=fit1;
                    pBest(i,j)=pop(i,j);
        ...
        for i=1:s_ps
            ...
            s_fitness(i)=power_allocation_3D(x);%***************************
...
%     xlabel('Generation');
%     ylabel('fitness');
%     title('Power Allocation CPSO-H-3D');
end
% hold off;
% figure;
% for i=1:(maxGen+1)
...
% end
% plot(AVE,'r');
% title('CPSO-H-3D MEAN');
% save('CPSO_H_3D_50times','COSTS')
ds = load('CPSO_H_3D');