...
data_12 = load('LPSO_3D');            % run the MATLAB file 'Local_PSO_3D.m' to generate this data
data_14 = load('LPSO_EVOLER_3D');     % run the MATLAB file 'EVOLER_L_3D.m' to generate this data
data_15 = load('downhill_EVOLER_3D'); % run the MATLAB file 'EVOLER_D_3D.m' to generate this data
...
Tao_1 = (8.233662879017250e+03)*(1+0.0001); 
...
for ii = 1 : size(diff_CPSO_H,1)
    ...
    genra_NP_PSO(ii,1) = k2(end)+1;
    genra_NP_PSO(ii,2) = data_NP_PSO(ii,k2(end)+1);
end
...
%% Figure 4-d: achievable minimal costs of different methods, and the probabilities of finding them
figure
