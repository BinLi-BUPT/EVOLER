...
Discret_leng = 160;                            % Discrete size of the original problem space; M=N=Discret_leng;
...
Discret_leng_int = [Discret_leng_now(1:Downsample_factor:end),Discret_leng]; % Down-sampling with the factor 80
...
for iiii = 1 : num_all
    ...
    for iii=1:1:length(Discret_leng_int)
        for jjj=1:1:length(Discret_leng_int)
            Z(iii,jjj)=rotated_non_continuous_rastrigin([x_axis(Discret_leng_int(iii)),y_axis(Discret_leng_int(jjj))],M); % rotated_non_continuous_rastrigin: a standard test function
        end   
    end
    ...
    s = length(Discret_leng_int)-1;                           % The sampling length s-O{rlog(r)}, r is the estimated rank of this problem space (see the online estimation of unknown rank value in Online_rank_estimation.m). 
    Index_c = randperm(length(Discret_leng_int),1);           
    C1_temp = Z(:,Index_c);                              
    c1_temp = inde_samd(C1_temp,Index_c,s);                   % Determining the row sampling index according to the randomly sampled columns 
    R1 = Z(c1_temp,:);
    c2_temp = inde_samd(R1',Index_c,s);                       % Determining the remaining colums sampling index according to the row sampled result
    C1 = Z(:,c2_temp);                                        
    
    %----------------------------------
    % step (ii): Reconstruction of the approximate problem space, via the special form: \hat(Z) = C1 * U1 * R1;
    U0 = Z(c1_temp,c2_temp);
    [U_u,S_u,V_u]=svd(U0);
    Uu=U_u';
    s_0= 1;
    ...
    N_add = round((s*(length(Discret_leng_int))+s*(length(Discret_leng_int))-s*s)/sizepop); % The overall required samples to reconstruct an attenion subspace, which is used in the final plot.  
    ...
    for i=1:n
        ...
        fit_S(i)=rotated_non_continuous_rastrigin(S(:,i)',M);
    end
   ...
   for i=1:maxGen
        ...
        fit_Vr=rotated_non_continuous_rastrigin(Vr',M);%***********************
        if fit_Vr<=fit_Vl % Expansion:he reflection perform berter than the best position. 
            ...
            fit_Ve=rotated_non_continuous_rastrigin(Ve',M);%******************
            if fit_Ve<fit_Vl
                S(:,1)=[];      S=[S,Ve];
        ...
        elseif fit_Vr<=fit_Vh % Contraction: the reflection performs better than the worst position but worse than the second worst position.
            ...
            fit_Vc=rotated_non_continuous_rastrigin(Vc',M);%*******************************
            if fit_Vc<fit_Vh % The contruction performs better than the worst position.
                ...
                for j=1:D
                    ...
                    fit_S(j)=rotated_non_continuous_rastrigin(S(:,j)',M);
                end
            end
