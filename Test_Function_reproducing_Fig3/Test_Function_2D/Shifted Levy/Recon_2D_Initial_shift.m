...
for iii=1:1:Discret_leng
    for jjj=1:1:length(Index_c)
        C1(iii,jjj)=shifted_levy([all_axis(1, iii),all_axis(2, Index_c(jjj))], shift_op);  % weierstrass: a standard test function
    end
end
for iii=1:1:length(Index_r)
    for jjj=1:1:Discret_leng
        R1(iii,jjj)=shifted_levy([all_axis(1, Index_r(iii)),all_axis(2, (jjj))], shift_op);  % weierstrass: a standard test function
    end
end
U0 = C1(Index_r,:);
U01 = R1(:,Index_c);
