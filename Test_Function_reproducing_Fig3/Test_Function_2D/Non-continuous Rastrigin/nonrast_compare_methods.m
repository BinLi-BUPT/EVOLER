...
%             yBest = xBest(nMin,:);
    ...
    for i=1:(D+1)
        ...
        fit_S(i)=non_continuous_rastrigin(S(:,i));
    end
    ...
    for i=1:maxGen
        ...
        fit_Vr=non_continuous_rastrigin(Vr);%***********************
        if fit_Vr<=fit_Vl % Expansion:he reflection perform berter than the best position. 
            ...
            fit_Ve=non_continuous_rastrigin(Ve);%******************
            if fit_Ve<fit_Vl
                S(:,1)=[];      S=[S,Ve];
        ...
        elseif fit_Vr<=fit_Vh % Contraction: the reflection performs better than the worst position but worse than the second worst position.
            ...
            fit_Vc=non_continuous_rastrigin(Vc);%*******************************
            if fit_Vc<fit_Vh % The contruction performs better than the worst position.
                ...
                for j=1:D
                    ...
                    fit_S(j)=non_continuous_rastrigin(S(:,j));
                end
            end
...
da_1_Pro = load('Pro_data_non_rast');  % run the file 'schewfel_EVOLER' to generate this data
...
xlim([0,maxGen]);
title('2-D non continuous rastrigin');
...
title('2-D non_continuous_rastrigin');
legend('Standard PSO','DPSO','CPSO-S','CPSO-H','CLPSO','MCPSO','NPSO','GCPSO','EVOLER','MPCPSO','DNSPSO','Local PSO','location','SouthWest');
save 'all_data.mat';
