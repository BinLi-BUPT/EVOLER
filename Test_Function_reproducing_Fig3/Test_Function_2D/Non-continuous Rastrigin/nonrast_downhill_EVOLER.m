...
for iii=1:1:Discret_leng
    for jjj=1:1:Discret_leng
        Z(iii,jjj)=non_continuous_rastrigin([x_axis(iii),y_axis(jjj)]); % non_continuous_rastrigin: a standard test function
    end
end
...
for iiii = 1 : num_all
    ...
    C1 = Z(:,Index_c);                                % Column sampling
    R1 = Z(Index_r,:);                                % Row sampling
    ...
    Z_est = C1*U1*R1;                                         % Reconstructing the problem space.
    ...
    N_add = round((s*(Discret_leng)+s*(Discret_leng)-s*s)/sizepop); % The overall required samples to reconstruct an attention subspace, which is used in the final plot.
