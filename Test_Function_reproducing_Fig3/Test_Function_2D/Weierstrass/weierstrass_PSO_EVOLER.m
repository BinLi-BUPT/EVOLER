...
Discret_leng = 100;                             % Discrete size of the original problem space; M=N=Discret_leng;
x_axis = linspace(popmin,popmax,Discret_leng);  
y_axis = linspace(popmin,popmax,Discret_leng);  

Discret_leng_now = 1:1:Discret_leng;  
Downsample_factor = 5;
Discret_leng_int = Discret_leng_now(1:Downsample_factor:end); % Down-sampling with the factor 5
...
for iii=1:1:length(Discret_leng_int)
    for jjj=1:1:length(Discret_leng_int)
        Z(iii,jjj)=weierstrass([x_axis(Discret_leng_int(iii)),y_axis(Discret_leng_int(jjj))]);  % weierstrass: a standard test function
    end
end
...
for iiii = 1 : num_all
    ...
    N_add = round((s*(length(Discret_leng_int))+s*(length(Discret_leng_int))-s*s)/N);  % The overall required samples to reconstruct an attention subspace, which is used in the final plot.
