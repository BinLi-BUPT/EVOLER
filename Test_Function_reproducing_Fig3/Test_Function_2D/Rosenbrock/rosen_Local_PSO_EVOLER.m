...
for iii=1:1:Discret_leng
    for jjj=1:1:Discret_leng
        Z(iii,jjj)=rosen([x_axis(iii),y_axis(jjj)]);  % rosen: a standard test function 
    end
end
   ...
   % Initialization of population, around the identified attention subspace
    ...
    for i=1:MaxDT
        for j=1:sizepop
            ...
            V(j,:)=w*V(j,:)+c1(i)*rand.*(pbest(j,:)-pop(j,:))+c2(i)*rand.*(lbest-pop(j,:));
...
Probability=p15;
figure
h15 = plot(mean_various,Probability,'x','MarkerSize',12,'lineWidth',2,'color',[0.15,0.15,0.15]);
