...
for iiii = 1 : num_all
   ...
   %% Step 1: Low-rank Representation Learning, which reconstructs the whole problem space from the very limited samples. 
    ...
    N_add = round((s*(Discret_leng_int_num)+s*(Discret_leng_int_num)-s*s)/popsize);  % The overall required samples to reconstruct an attention subspace, which is used in the final plot.  
