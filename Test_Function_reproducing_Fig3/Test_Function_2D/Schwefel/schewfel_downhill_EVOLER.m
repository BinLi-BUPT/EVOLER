...
popMin=-500;                                   % Variable Range [popmin: popmax]
proposed_downhill_16=zeros(1,maxGen);

Discret_leng = 20;                             % Discrete size of the original problem space; M=N=Discret_leng;
...
for iii=1:1:Discret_leng
    for jjj=1:1:Discret_leng
        Z(iii,jjj)=schewfel([x_axis(iii),y_axis(jjj)]);  % schewfel: a standard test function
    end
end

num_all = 500; % The number of simulation trials
Fitness_all = zeros(maxGen,num_all);
for iiii = 1 : num_all
    iiii
   ...
   PDF_num_proposed_downhill(iiii) = PDF_cal16;
    ...
    Y = 200*ones(1,maxGen+1);          % Assuming the large fitness in the first reconstruction stage
