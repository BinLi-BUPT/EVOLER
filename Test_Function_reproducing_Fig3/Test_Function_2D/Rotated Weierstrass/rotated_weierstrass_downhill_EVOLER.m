...
Discret_leng = 1000;                           % Discrete size of the original problem space; M=N=Discret_leng;
...
Discret_leng_int = Discret_leng_now(1:Downsample_factor:end);  % Down-sampling with the factor 20
...
for iiii = 1 : num_all
    ...
    for iii=1:1:length(Discret_leng_int)
        for jjj=1:1:length(Discret_leng_int)
            Z(iii,jjj)=rotated_weierstrass([x_axis(Discret_leng_int(iii)),y_axis(Discret_leng_int(jjj))],M); % rotated_weierstrass: a stardand test function
        end   
    end
   ...
   %% Step 1: Low-rank Representation Learning, which reconstructs the whole problem space from the very limited samples. 
    ...
    s = 40;                                          % The sampling length s-O{rlog(r)}, r is the estimated rank of this problem space (see the online estimation of unknown rank value in Online_rank_estimation.m). 
    ...
    N_add = round((s*(Discret_leng_int_num)+s*(Discret_leng_int_num)-s*s)/sizepop);  % The overall required samples to reconstruct an attention subspace, which is used in the final plot.  
