...
%             yBest = xBest(nMin,:);
    ...
    for i=1:(D+1)
        ...
        fit_S(i)=rotated_weierstrass(S(:,i)',M);
    end
    ...
    for i=1:maxGen
        ...
        fit_Vr=rotated_weierstrass(Vr',M);%***********************
        if fit_Vr<=fit_Vl % Expansion:he reflection perform berter than the best position. 
            ...
            fit_Ve=rotated_weierstrass(Ve',M);%******************
            if fit_Ve<fit_Vl
                S(:,1)=[];      S=[S,Ve];
        ...
        elseif fit_Vr<=fit_Vh % Contraction: the reflection performs better than the worst position but worse than the second worst position.
            ...
            fit_Vc=rotated_weierstrass(Vc',M);%*******************************
            if fit_Vc<fit_Vh % The contruction performs better than the worst position.
                ...
                for j=1:D
                    ...
                    fit_S(j)=rotated_weierstrass(S(:,j)',M);
                end
            end
...
da_1_Pro = load('Pro_data_rotweier');  % run the file 'schewfel_EVOLER' to generate this data
