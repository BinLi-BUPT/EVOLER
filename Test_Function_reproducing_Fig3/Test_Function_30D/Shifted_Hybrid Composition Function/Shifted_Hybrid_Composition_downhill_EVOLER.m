...
for iiii = 1 : num_all
    ...
    %----------------------------------
    shift_op = (-0.5+1*rand(1,D));             % Shifted global optimum, range [-0.5,0.5]
    Discret_leng = 4;                          % Discrete size of the original problem space;                              
    Trail_times = 5;                           % The times of hierarchy reconstruction strategy, to reduce the problem space search range
    ...
    for ii  = 1 : Trail_times
        [global_o,pop_div1,pop_div2] = Search_30D_Initial(max_ite,min_ite,s-1,Discret_leng,shift_op);
        max_ite = pop_div1;
        min_ite = pop_div2;
    end
    ...
    Trail_times2 = 10;                        % The times of hierarchy reconstruction strategy based on low-rank sampling
    for ii = 1 : Trail_times2
        [global_optimum,pop_div1,pop_div2] = Recon_30D_Initial(max_ite,min_ite,s,Discret_leng,shift_op);
        max_ite = pop_div1;
        min_ite = pop_div2;
    end    

    N_add = round(Trail_times*(5*Discret_leng^6*(s-1)^4-5*(s-1)^6)/sizepop)+...
       round(Trail_times2*((4*Discret_leng*s^4-3*s^5 + Discret_leng^2*s^5)*2+...
    ...
    for i=1:D+1
        ...
        fit_S(i)=shifted_hybrid_func1(S(:,i)',shift_op);
    end
   ...
   for i=1:maxGen
        ...
        fit_Vr=shifted_hybrid_func1(Vr',shift_op);%***********************
        if fit_Vr<=fit_Vl % Expansion:he reflection perform berter than the best position. 
            ...
            fit_Ve=shifted_hybrid_func1(Ve',shift_op);%******************
            if fit_Ve<fit_Vl
                S(:,1)=[];      S=[S,Ve];
        ...
        elseif fit_Vr<=fit_Vh % Contraction: the reflection performs better than the worst position but worse than the second worst position.
            ...
            fit_Vc=shifted_hybrid_func1(Vc',shift_op);%*******************************
            if fit_Vc<fit_Vh % The contruction performs better than the worst position.
                ...
                for j=1:D
                    ...
                    fit_S(j)=shifted_hybrid_func1(S(:,j)',shift_op);
                end
            end
...
if isempty(PDF_num_proposed_downhill_16)
    PDF_num_proposed_downhill_16 = maxGen+N_add;
    p16 = 0;
else
