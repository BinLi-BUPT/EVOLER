...
for iiii = 1 : num_all
    ...
    Trail_times = 18;                          % The times of hierarchy reconstruction strategy, to reduce the problem space search range
    ...
    Discret_leng1 = Discret_leng+1;            % Discrete size of the original problem space;
    Trail_times2 = 3;                          % The times of hierarchy reconstruction strategy based on low-rank sampling
    ...
    N_add = round(Trail_times*(5*Discret_leng^6*(s-1)^4-5*(s-1)^6)/sizepop)+...
       round(Trail_times2*((4*Discret_leng1*s^4-3*s^5 + Discret_leng1^2*s^5)*2+...
...
if isempty(PDF_num_proposed_downhill_16)
    PDF_num_proposed_downhill_16 = N_add+maxGen;
    p16 = 0;
else
