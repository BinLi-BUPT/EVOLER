...
popMin=-popMax;
maxGen=1100;
des_val=1e-8;  %If the output result is less than the threshold 'des_val', the global optimum is found  
vMax=0.15*(popMax); 
vMin=(-1)*vMax;
...
%             yBest = xBest(nMin,:);
        ...
        %second insert
        speed = floor(speed * w + c1 * rand * (xBest - pop) + c2 * rand * (repmat(yBest,ps,1) - pop));%renew speed
...
%         end
%         fit_S(i)=rotated_griewank(reshape(S(:,i),1,D),M);
%     end
...
%             Vr(d)=max(xMin,Vr(d));
%         end
%         fit_Vr=rotated_griewank(reshape(Vr,1,D),M);%***********************
...
%                 Ve(d)=max(xMin,Ve(d));
%             end
%             fit_Ve=rotated_griewank(reshape(Ve,1,D),M);%******************
%             if fit_Ve<fit_Vl
%                 S(:,1)=[];      S=[S,Ve];
...
%                 Vc(d)=max(xMin,Vc(d));
%             end
%             fit_Vc=rotated_griewank(reshape(Vc,1,D),M);%*******************************
...
%                     end
%                     fit_S(j)=rotated_griewank(reshape(S(:,j),1,D),M);
%                 end
%             end
...
% hold on;
ylabel('Fitness');
legend('PSO','DPSO','CPSO-S','CPSO-H','CLPSO','MCPSO','NPSO','GCPSO','EVOLER','MPCPSO','DNSPSO','Local PSO','location','SouthWest');
xlim([0,maxGen]);
% title('2-D levy');
...
mean_various = [mean(PDF_num_SPSO_1),mean(PDF_num_DPSO_2),mean(PDF_num_CPSO_3),mean(PDF_num_CPSOH_4),mean(PDF_num_CLPSO_5),mean(PDF_num_MLPSO_6),mean(PDF_num_NPSO_7),mean(PDF_num_GCPSO_8),mean(X1),mean(PDF_num_MPCPSO_10),mean(PDF_num_DNSPSO_11),mean(PDF_num_LPSO_12)];
Probablity = [p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12];
figure(2)
