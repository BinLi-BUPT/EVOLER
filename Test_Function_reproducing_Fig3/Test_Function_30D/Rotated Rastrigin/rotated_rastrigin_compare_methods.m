...
%             yBest = xBest(nMin,:);
    ...
    for i=1:(D+1)
        ...
        fit_S(i)=rotated_rest(reshape(S(:,i),1,D),M);
    end
    ...
    for i=1:maxGen
        ...
        fit_Vr=rotated_rest(reshape(Vr,1,D),M);%***********************
        if fit_Vr<=fit_Vl % Expansion:he reflection perform berter than the best position. 
            ...
            fit_Ve=rotated_rest(reshape(Ve,1,D),M);%******************
            if fit_Ve<fit_Vl
                S(:,1)=[];      S=[S,Ve];
        ...
        elseif fit_Vr<=fit_Vh % Contraction: the reflection performs better than the worst position but worse than the second worst position.
            ...
            fit_Vc=rotated_rest(reshape(Vc,1,D),M);%*******************************
            if fit_Vc<fit_Vh % The contruction performs better than the worst position.
                ...
                for j=1:D
                    ...
                    fit_S(j)=rotated_rest(reshape(S(:,j),1,D),M);
                end
            end
...
da_1_Pro = load('Pro_data_rotrest');  
...
hold on;
ylabel('Fitness');
legend('PSO','DPSO','CPSO-S','CPSO-H','CLPSO','MCPSO','NPSO','GCPSO','EVOLER','MPCPSO','DNSPSO','Local PSO','Downhill','location','SouthWest');
xlim([0,maxGen]);
% title('2-D levy');
