...
popMin=-popMax;
maxGen=1800;
des_val= 8e-3;  %If the output result is less than the threshold 'des_val', the global optimum is found  
vMax=0.15*(popMax); 
vMin=(-1)*vMax;
...
for run=1:num_all
    ...
    for i=1:ps
        ...
        initFitness(i)=rastrigin_noise(initPop(i,:));%*****************************************************************
    end
    initpBest=initPop;
    ...
    for i=1:maxGen
        for j=1:ps
            ...
            fitness(j)=rastrigin_noise(pop(j,:));%************************************************************************
        ...
        for j=1:D
            for i=1:ceil(ps/D)
                ...
                temp1(j)=pop(i,j);pBest1(j)=pBest(j);
                 fit1=rastrigin_noise(temp1);
                 fit2=rastrigin_noise(pBest1);%***************************************************************
                if fit1<fit2
                    pBestFit(i)=fit1;
                    pBest(i,j)=pop(i,j);
                end
    ...
    for k=1:maxGen
        for j=1:D
            for i=1:p_s
                ...
                temp1=gBest;pBest1=gBest;
                temp1(j)=pop(i,j);pBest1(j)=pBest(j);
                fit1=rastrigin_noise(temp1);fit2=rastrigin_noise(pBest1);%****************************************************
                if fit1<fit2
                    pBestFit(i)=fit1;
                    pBest(i,j)=pop(i,j);
                end
        ...
        for i=1:p_s
            ...
            s_fitness(i)=rastrigin_noise(s_pop(i,:));%*************************************************************
    ...
    for k=1:maxGen
        for i=1:ps
            ...
            fitness(i)=rastrigin_noise(pop(i,:));%****************************************************************
    ...
    for i=1:maxGen
        for j=1:groupNum-1
            for k=1:p_s
                ...
                Fitness(j,k)=rastrigin_noise(reshape(Pop(j,k,:),1,D));%******************************************************************************
        ...
        for k=1:p_s
            ...
            fit=rastrigin_noise(reshape(Pop(groupNum,k,:),1,D));%*************************************************************************************
    ...
    for iIteration = 1:maxGen
        for iParticles = 1:ps
            ...
            xFitness=rastrigin_noise(pop(iParticles,:));    
...
%             yBest = xBest(nMin,:);
    ...
    for i=1:maxGen
        ...
        for j=1:N2
            ...
            fitness(index)=rastrigin_noise(pop(index,:));%*****************************************************************
...
da_1_Pro = load('Pro_data_rast_noise');  
