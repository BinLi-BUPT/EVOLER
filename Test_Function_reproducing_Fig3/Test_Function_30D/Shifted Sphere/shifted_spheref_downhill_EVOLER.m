...
for iiii = 1 : num_all
    ...
    Trail_time = 20;                           % The times of hierarchy reconstruction
    ...
    for i=1:D+1
        ...
        fit_S(i)=shifted_spheref(S(:,i)',shift_op);
    end
   ...
   for i=1:maxGen
        ...
        fit_Vr=shifted_spheref(Vr',shift_op);%***********************
        if fit_Vr<=fit_Vl % Expansion:he reflection perform berter than the best position. 
            ...
            fit_Ve=shifted_spheref(Ve',shift_op);%******************
            if fit_Ve<fit_Vl
                S(:,1)=[];      S=[S,Ve];
        ...
        elseif fit_Vr<=fit_Vh % Contraction: the reflection performs better than the worst position but worse than the second worst position.
            ...
            fit_Vc=shifted_spheref(Vc',shift_op);%*******************************
            if fit_Vc<fit_Vh % The contruction performs better than the worst position.
                ...
                for j=1:D
                    ...
                    fit_S(j)=shifted_spheref(S(:,j)',shift_op);
                end
            end
