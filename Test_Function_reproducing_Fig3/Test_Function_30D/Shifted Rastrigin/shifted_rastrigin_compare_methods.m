...
popMin=-popMax;
maxGen=1000;
des_val= 1e-6;  %If the output result is less than the threshold 'des_val', the global optimum is found  
vMax=0.15*(popMax); 
vMin=(-1)*vMax;
...
%             yBest = xBest(nMin,:);
    ...
    for i=1:(D+1)
        ...
        fit_S(i)=shifted_rastrigin(S(:,i),M);
    end
        ...
        elseif fit_Vr<=fit_Vh % Contraction: the reflection performs better than the worst position but worse than the second worst position.
            ...
            if fit_Vc<fit_Vh % The contruction performs better than the worst position.
                ...
                for j=1:D
                    ...
                    fit_S(j)=shifted_rastrigin(S(:,j),M);
                end
            end
...
da_1_Pro = load('shifted_Pro_data_rast');  
