...
%----------Fig. 2-(e): the convergence of EVOLER method for such one non-convex 2D Schwefel function
close all
figure
...
%----------Fig. 2-(f): the convergence Generations of our proposed EVOLER method, which greatly reduce the complexity;
Convergence_point = [];
for kkkkk=1:num_all
    fitness_current = Fitness_all(:,kkkkk);
    x_convergent = find(fitness_current<2.8e-5);
    if (~isempty(x_convergent))
        Convergence_point = [Convergence_point; min(x_convergent)];
    end
end
[Yy1,Xx1] = hist(Convergence_point,10);
figure
hold on
bar(Xx1,Yy1)
