...
% To minimize the complex interaction between MATLAB and LUMERICAL, we provide the data of 5-dimensional tensor--'single_trans_results', 
...
A_end = load('single_trans_results');
A_ten_end = tensor(A_end.data_rec_1);

%% The singular values of the problem space (d = 5)
s11 = tenmat(A_ten_end,1);
[~,sig_1,~] = svd(double(s11),'econ');
figure
semilogy(diag(sig_1))
s11 = tenmat(A_ten_end,2);
[~,sig_1,~] = svd(double(s11),'econ');
hold on
semilogy(diag(sig_1))
s11 = tenmat(A_ten_end,3);
[~,sig_1,~] = svd(double(s11),'econ');
hold on
semilogy(diag(sig_1))
s11 = tenmat(A_ten_end,4);
[~,sig_1,~] = svd(double(s11),'econ');
hold on
semilogy(diag(sig_1))
s11 = tenmat(A_ten_end,5);
[~,sig_1,~] = svd(double(s11),'econ');
hold on
semilogy(diag(sig_1))
...
A = data_rec_ten;
A_end_end = [];
tao = 0.955;        % Threshold (>0.9) of accumulated energy of the first r singular values, to estimate the rank of problem space and configure a sampling length.
...
T_11 = [15:15:150];
index_1_est = flip(T_11(m_all));                % the center of attention subspace used for LUMERICAL simulations
...
save('index_1_est','index_1_est');              % Save the estimated global optimum.
