...
% Rank estimation demo for ackley function

close all
clear all
popmax=32;                                               % Parameter range of each dimension
popmin=-32;                                              % Parameter range of each dimension 
Vmax=0.1*popmax;
Vmin=0.1*popmin;
...
for iii=1:1:Discret_leng
    for jjj=1:1:Discret_leng
        Z(iii,jjj)=ackley([x_axis(iii),y_axis(jjj)]);    % Evalutation of test function, ackley function
    end
end
[m,n] = size(Z);
figure
